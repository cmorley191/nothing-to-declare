...
(define (stamper-setup-env background-filename foreground-filenames)
  (let* (; source-layers-image just serves to hold the foreground-layers, but we start by opening background for consistent image properties
         ...
         (template-image (car (gimp-image-duplicate source-layers-image)))
         (placeholder-background-layer (car (gimp-image-get-active-layer template-image)))
         (foreground-layers
           (stamper-helper-map
    ...
    (gimp-image-scale template-image 140 140)

    (let* ((background-layer (car (gimp-layer-new-from-drawable source-layers-image-background-layer template-image)))
      ...
      (gimp-image-insert-layer template-image background-layer 0 0)
      (gimp-image-remove-layer template-image placeholder-background-layer)
      (gimp-layer-set-offsets background-layer 20 20)
      ...
      (gimp-image-select-color template-image CHANNEL-OP-REPLACE background-layer (list 0 0 0))
      (gimp-selection-invert template-image)
      (gimp-drawable-edit-clear background-layer)
      (gimp-selection-none template-image)
      ...
      (let* ((width (car (gimp-image-width template-image)))
             (height (car (gimp-image-height template-image)))
             (alpha-type 
              (cond 
                ((= type RGB-IMAGE) RGBA-IMAGE)
                ((= type RGBA-IMAGE) RGBA-IMAGE)
                ((= type GRAY-IMAGE) GRAYA-IMAGE)
                ((= type GRAYA-IMAGE) GRAYA-IMAGE)
                )
              )
             (bump-mask-layer (car (gimp-layer-new template-image width height alpha-type "mask layer" 100 LAYER-MODE-NORMAL-LEGACY)))
        ...
        (if (= bump? FALSE)
          (gimp-item-delete bump-mask-layer)
          ; else:
          (begin
            (gimp-drawable-fill bump-mask-layer FILL-WHITE)
            (gimp-image-insert-layer template-image bump-mask-layer 0 0)
            (let* ((mask (car (gimp-layer-create-mask bump-mask-layer ADD-MASK-WHITE)))
                   )
              (gimp-layer-add-mask bump-mask-layer mask)
              (plug-in-plasma 1 template-image mask bump-seed bump-granularity)
              (if (> bump-smoothness 0)
                (plug-in-gauss-rle 1 template-image mask bump-smoothness TRUE TRUE)
                )
              (gimp-layer-set-opacity bump-mask-layer 50)
              )
            )
          )
        (gimp-layer-add-mask bump-mask-layer bump-mask-mask)
        (plug-in-plasma 1 template-image bump-mask-mask bump-seed bump-granularity)
        (plug-in-gauss-rle 1 template-image bump-mask-mask bump-smoothness TRUE TRUE)
        (gimp-layer-set-opacity bump-mask-layer 50)



        (list template-image background-layer source-layers-image foreground-layers)
        )
      )
    )
  )
